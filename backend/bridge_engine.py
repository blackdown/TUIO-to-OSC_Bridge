...
class BridgeEngine:
    ...
    def _process_frame(self, frame: dict) -> None:
        ...
        active_count = len(frame.get("cursors", {})) + len(frame.get("objects", {}))
    ...
    def _send_objects(
        ...
        # Objects use a separate base path derived from template
    ...
    def _send_removed(
        ...
        for sid in frame.get("removed", frozenset()):
            # Send alive=0 on cursor base path (we can't know if it was cursor or object)
            base_cur = template.replace("{id}", str(sid))
            self._send(client, f"{base_cur}/alive", 0)
            obj_template = template.replace("cursor", "object")
            base_obj = obj_template.replace("{id}", str(sid))
            self._send(client, f"{base_obj}/alive", 0)
