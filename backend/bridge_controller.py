...
class BridgeController(QObject):
    ...
    def loadConfig(self) -> None:
        self._config_mgr.load()
        self.targetCountChanged.emit()
        self.configChanged.emit()

    @Property(int, notify=targetCountChanged)
    def targetCount(self) -> int:
        return len(self._config_mgr.config.get("osc_output", {}).get("targets", []))

    @Slot()
    def addTarget(self) -> None:
        self._config_mgr.add_target()
        self._engine.configure(self._config_mgr.config)
        self.targetCountChanged.emit()
        self.configChanged.emit()
    ...
    def removeTarget(self, index: int) -> None:
        if self._config_mgr.remove_target(index):
            self._engine.configure(self._config_mgr.config)
            self.targetCountChanged.emit()
            self.configChanged.emit()
