...
class LogModel(QAbstractListModel):
    ...
    def add_entry(self, address: str, value: object) -> None:
        ...
        # Track whether the deque will drop its oldest entry
        was_full = len(self._entries) == self.MAX_ENTRIES
        dropped = self._entries[0] if was_full else None
        self._entries.append(entry)

        # If the dropped entry was visible, remove it from the top
        if dropped is not None and self._visible and self._visible[0] is dropped:
        ...
        f = self._filter
        if not f or f in address.lower():
